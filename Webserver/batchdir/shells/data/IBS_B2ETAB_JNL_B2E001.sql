...
spool ibs_b2etab_jnl_b2e001.tmp
set trimspool on
select
...
trim(replace(replace(JNL_REJMSG,chr(13)),chr(10)))||'|+|'||BATCH_ID||'|+|'||CLIENTPATCHID||'|+|'||ACNO||'|+|'||AMOUNT||'|+|'||CURRENCY||'|+|'||
FIXEDDATELEN||'|+|'||EXTENDFLG||'|+|'||to_char(CREATE_TIME,'yyyyMMdd hh24:mi:ss')||'|+|'
from IBS_B2ETAB_JNL_B2E001 
