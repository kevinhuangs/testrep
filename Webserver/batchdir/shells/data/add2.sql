...
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理开始(IBS_MTAB_PJNLASSETPOOL增量导出) IBS_MTAB_PJNLASSETPOOL*****' from dual;
spool off
@IBS_MTAB_PJNLASSETPOOL.sql
spool /backup/batchdir/shells/data/add_log/2144IBS_MTAB_PJNLASSETPOOL.log
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理结束(IBS_MTAB_PJNLASSETPOOL增量导出) IBS_MTAB_PJNLASSETPOOL*****' from dual;
spool off
...
#add by pengxiaomei 20131030-----start
#增加增量 IBS_MTAB_MP_PJNL(微信银行交易类日志)    20131030              每日增量
spool /backup/batchdir/shells/data/add_log/2165IBS_MTAB_MP_PJNL_STARTTIME.log
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理开始(IBS_MTAB_MP_PJNL增量导出) IBS_MTAB_MP_PJNL*****' from dual;
spool off
@IBS_MTAB_MP_PJNL.sql
spool /backup/batchdir/shells/data/add_log/2166IBS_MTAB_MP_PJNL_ENDTIME.log
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理结束(IBS_MTAB_MP_PJNL增量导出)IBS_MTAB_MP_PJNL*****' from dual;
spool off
#add by pengxiaomei 20131030------end

#add by chenyanan 20131030-----start
#增加增量 IBS_MTAB_MP_PJNLQUERY(微信银行查询交易日志表)    20131030              每日增量
spool /backup/batchdir/shells/data/add_log/2167IBS_MTAB_MP_PJNLQUERY_STARTTIME.log
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理开始(IBS_MTAB_MP_PJNLQUERY增量导出) IBS_MTAB_MP_PJNLQUERY*****' from dual;
spool off
@IBS_MTAB_MP_PJNLQUERY.sql
spool /backup/batchdir/shells/data/add_log/2168IBS_MTAB_MP_PJNLQUERY_ENDTIME.log
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理结束(IBS_MTAB_MP_PJNLQUERY增量导出) IBS_MTAB_MP_PJNLQUERY*****' from dual;
spool off
#add by chenyanan 20131030------end
