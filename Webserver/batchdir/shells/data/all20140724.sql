...
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理开始(IBS_MTAB_MP_PJNLASSETPOOL全量导出) IBS_MTAB_MP_PJNLASSETPOOL*****' from dual;
spool off
@IBS_MTAB_MP_PJNLASSETPOOL20140724.sql
spool /backup/batchdir/shells/data/all_log/3118IBS_MTAB_MP_PJNLASSETPOOL.log
set trimspool on
select '*****'||to_char(sysdate,'yyyyMMdd hh24:mi:ss')||' STEP处理结束(IBS_MTAB_MP_PJNLASSETPOOL全量导出) IBS_MTAB_MP_PJNLASSETPOOL*****' from dual;
spool off
