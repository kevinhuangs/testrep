...
echo "*****${nowTime} STEP[1]处理开始(手工同步证书缴费账单)ByHandCreateCertBillStart*****" >>/backup/batchdir/log/tt_run.log
/opt/java6/bin/java -Xms1024m -Xmx1024m -classpath $CLASSPATH com.ceb.ibs.ByHandCreateCertBillStart
nowTime=`sh /backup/batchdir/shells/data/addTime.sh 0`
echo "*****${nowTime} STEP[1]处理结束(手工同步证书缴费账单)ByHandCreateCertBillStart*****" >>/backup/batchdir/log/tt_run.log
