...
countFile=`ls /backup/batchdir/anbmx/SynCreditLogo20130328 | grep "CREDIT_DELE_AC_LOGO_20130328.txt" |wc -l`
if [ countFile  -eq 0 ]; then
echo "本地不存在数据文件CREDIT_DELE_AC_LOGO_20130328.txt"   >>/backup/batchdir/log/SynCreditLogo.log
else
#执行batch类更新本地数据库（批量同步logo）
#如果不存在sql_delelogo目录，新建sql_delelogo目录用于存放生产的sql文件

countsql=`ls /backup/batchdir/anbmx/SynCreditLogo20130328| grep "sql_delelogo" |wc -l`
if [ countsql  -eq 0 ]; then
echo "本地新建sql_delelogo子目录"   >>/backup/batchdir/log/SynCreditLogo.log
cd /backup/batchdir/anbmx/SynCreditLogo20130328
mkdir sql_delelogo
else
echo "本地已存在sql_delelogo子目录"   >>/backup/batchdir/log/SynCreditLogo.log
fi
...
echo "*****${nowTime} JOB处理开始(批量同步乐惠金卡logo开始) SynDeleCreditLogo.sh*****" >>/backup/batchdir/log/SynCreditLogo.log
/opt/java6/bin/java -Xms512m -Xmx512m -classpath $CLASSPATH  com.ceb.ibs.batch.SynDeleCreditLogo>> /backup/batchdir/log/SynCreditLogo.log
nowTime=`sh /backup/batchdir/shells/data/addTime.sh 0`
echo "*****${nowTime} JOB处理结束(批量同步乐惠金卡logo完成) SynDeleCreditLogo.sh*****" >>/backup/batchdir/log/SynCreditLogo.log
