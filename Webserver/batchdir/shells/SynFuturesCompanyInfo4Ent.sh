...
CTL=/backup/batchdir/ctrls/INTOFUTURESCOMPANYINFO4ENT.ctl
TABLEMAME=IBS_ETAB_FUTURESCOMPANY_TMP
SYNSQL=/backup/batchdir/shells/SynFuturesCompanyInfo4Ent.sql
...
　　SELECT COUNT( * ) COUN FROM IBS_ETAB_FUTURESCOMPANY_TMP;
　　exit;
　　END`
...
echo "NTEBANK-SUCCESS 表${TABLEMAME}导入数据成功SynFuturesCompanyInfo4Ent.sh" 
else
echo "NTEBANK-ERROR 表${TABLEMAME}导入数据失败SynFuturesCompanyInfo4Ent.sh" 
fi
else 
