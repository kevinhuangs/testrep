...
TableName=IBS_ETAB_BIN
DBUSER=bankdb
DBPWD=bankdb
CTL=/backup/batchdir/ctrls/INTOBIN.ctl
...
　　SELECT COUNT( * ) COUN FROM IBS_ETAB_BIN_TMP;
　　exit;
　　END`
sqlline2=`expr $VALUE`
...
DELETE FROM IBS_ETAB_BIN;
INSERT INTO IBS_ETAB_BIN
       (ISSUER_CODE, ISSUER_NAME, CARDNO_LEN, BIN_NUM, CUPLOGO_FLAG, DEBITCREDIT_FLAG, ADDDELETE_STAT, ADDITIONAL_DATA)
(SELECT ISSUER_CODE, ISSUER_NAME, CARDNO_LEN, BIN_NUM, CUPLOGO_FLAG, DEBITCREDIT_FLAG, ADDDELETE_STAT, ADDITIONAL_DATA FROM IBS_ETAB_BIN_TMP);
COMMIT;
!
