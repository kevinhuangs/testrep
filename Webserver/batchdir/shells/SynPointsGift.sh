...
echo "****************下面判断是否存在数据文件***********************" >>/backup/batchdir/log/PointsGiftSynAction.log
countFile=`echo "\n" >>${DATAPATH}/fileName.txt|sed /^[[:space:]]*$/d ${DATAPATH}/fileName.txt|wc -l`
if [ countFile  -eq 0 ]; then
echo "本次没有生成数据文件"   >>/backup/batchdir/log/PointsGiftSynAction.log
else
#执行batch类更新本地数据库
#备份日志
logDay=`sh /backup/batchdir/shells/data/addday.sh -1 -`
mv /backup/batchdir/log/PointsGiftSynAction.log /backup/batchdir/log/PointsGiftSynAction.log.${logDay}
...
echo "*****${nowTime} JOB处理开始(网银积分兑换礼物同步开始) SynPointsGift.sh*****" >>/backup/batchdir/log/PointsGiftSynAction.log
/opt/java6/bin/java -Xms512m -Xmx512m -classpath $CLASSPATH  com.ceb.ibs.PointsGiftSynStart>> /backup/batchdir/log/PointsGiftSynAction.log 
nowTime=`sh /backup/batchdir/shells/data/addTime.sh 0`
echo "*****${nowTime} JOB处理结束(网银积分兑换礼物同步完成) SynPointsGift.sh*****" >>/backup/batchdir/log/PointsGiftSynAction.log
