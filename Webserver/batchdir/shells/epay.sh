...
FTPUSER=ftpuser
FTPPWD=ftpuser
FTPSERVPATH=/app/epay/data/protocol/server/
#FTPSERVIP=
#FTPUSER=
#FTPPWD=
#FTPSERVPATH=
...
#CIFJOINHOME=
CIFJOINHOME=/backup/batchdir/epaydata
...
cd $FTPSERVPATH
pwd
get ZF_${YESTODAY}_AGREEMENT.tar
close
bye
!
...
echo "==>压缩文件所在目录"
pwd
tar xvf ZF_${YESTODAY}_AGREEMENT.tar

ls ZF_${YESTODAY}_AGREEMENT
#echo "==>移动解压后的文件到epaydata目录"
#mv ZF_${YESTODAY}_AGREENENT/content.txt $CIFJOINHOME
#rm -r 
echo "==>清除临时文件"
pwd
rm -r ${CIFJOINHOME}/ZF_${YESTODAY}_AGREEMENT.tar
cd ..
