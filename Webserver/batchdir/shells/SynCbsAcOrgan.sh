...
echo "****************下面判断是否存在数据文件***********************" >>/backup/batchdir/log/SynCbsAcOrgan.log
countFile=`ls /backup/batchdir/anbmx/SynCbsAcOrgan20130328 | grep "CBS_AC_ORGAN_20130328.txt" |wc -l`
if [ countFile  -eq 0 ]; then
echo "本地不存在数据文件CBS_AC_ORGAN_20130328.txt"   >>/backup/batchdir/log/SynCbsAcOrgan.log
else
#执行batch类更新本地数据库（批量改开户机构）
#如果不存在sql目录，新建sql目录用于存放生产的sql文件

countsql=`ls /backup/batchdir/anbmx/SynCbsAcOrgan20130328| grep "sql" |wc -l`
if [ countsql  -eq 0 ]; then
echo "本地新建sql子目录"   >>/backup/batchdir/log/SynCbsAcOrgan.log
cd /backup/batchdir/anbmx/SynCbsAcOrgan20130328
mkdir sql
else
echo "本地已存在sql子目录"   >>/backup/batchdir/log/SynCbsAcOrgan.log
fi
...
echo "*****${nowTime} JOB处理开始(批量改机构号开始) SynCbsAcOrgan.sh*****" >>/backup/batchdir/log/SynCbsAcOrgan.log
/opt/java6/bin/java -Xms512m -Xmx512m -classpath $CLASSPATH  com.ceb.ibs.batch.SynCbsAcOrgan>> /backup/batchdir/log/SynCbsAcOrgan.log 
nowTime=`sh /backup/batchdir/shells/data/addTime.sh 0`
echo "*****${nowTime} JOB处理结束(批量改机构号完成) SynCbsAcOrgan.sh*****" >>/backup/batchdir/log/SynCbsAcOrgan.log
