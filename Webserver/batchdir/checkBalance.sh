...
mv /backup/batchdir/log/checkBalance.log /backup/batchdir/log/checkBalance.log.${logDay}
...
#echo "`date` ========================checkBalance Start================>" >> /backup/batchdir/log/checkBalance.log
nowTime=`sh /backup/batchdir/shells/data/addTime.sh 0`
echo "*****${nowTime} JOB处理开始(网银对账签约表批量) checkBalance.sh*****" >>/backup/batchdir/log/checkBalance.log
/opt/java6/bin/java -Xms512m -Xmx512m -classpath $CLASSPATH  com.ceb.ibs.CheckBalanceStart>> /backup/batchdir/log/checkBalance.log >> /backup/batchdir/log/checkBalance.log 
nowTime=`sh /backup/batchdir/shells/data/addTime.sh 0`
echo "*****${nowTime} JOB处理结束(网银对账签约表批量) checkBalance.sh*****" >>/backup/batchdir/log/checkBalance.log
#echo "`date` ========================checkBalance End=================>" >> /backup/batchdir/log/checkBalance.log